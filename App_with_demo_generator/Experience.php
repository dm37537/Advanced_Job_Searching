<?php
	...
	if ($mode=='Demo')
	{
		...
		$endDate = '01/01/2013';
		$jobDescription = 'Nurse in Oncology';
...
<html> 
	...
	<div>
		...
		<input type='button' name='cancel' value='Cancel' onclick=<?php if (isset($_SESSION['location'])) echo "location.href='" . $_SESSION['location']."'"; else echo "'history.back();'" ?> />
		 <input type="button" name='GenerateDemo' value="GenerateDemo" onclick="location.href='Experience.php?mode=Demo&resumeID=<?php echo $resumeID;?>&userID=<?php echo $userID;?>'" />
		</form>
	</div>
	<?php oci_close($conn); ?>
</html>